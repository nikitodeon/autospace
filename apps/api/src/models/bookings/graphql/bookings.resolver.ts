...
export class BookingsResolver {
  ...
  async bookingsForValet(
    ...
    return this.bookingsService.findAll({
      ...
      where: {
        ...args.where,
        Slot: {
          is: { Garage: { is: { companyId: { equals: company?.id } } } },
        },
      },
    })
  }
  ...
  async bookingsForGarage(
    ...
    const garage: any = await this.prisma.garage.findUnique({
      where: { id: garageId },
  ...
  async removeBooking(
    ...
    const booking: any = await this.prisma.booking.findUnique(args)
    checkRowLevelPermission(user, booking.customerId)
